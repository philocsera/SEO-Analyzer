...
GEO(Generative Engine Optimization, 생성형 검색엔진 최적화) 분석은 이미 구현되어 `/geo`에서 제공됩니다 — 휴리스틱 점수(Citability·Quotability·Specificity·Extractability) + 크롤러 게이트 + LLM 정성 리뷰 + 다중 URL 비교(`/geo/compare`) + 사이트 단위 진단(`/geo/site`) + 결과 공유 링크(`/geo/result/[id]`).

다음 기능을 추후 추가할 예정입니다:

- **GEO 검증 모드 (실측 인용률)**: 현재 GEO 점수는 휴리스틱입니다. 실제 LLM(ChatGPT·Claude 등)에 시뮬레이션 질문을 질의해 해당 페이지/도메인이 AI 답변에 **실제로 인용되는지 표본 측정**하는 모드를 추가할 예정입니다. 휴리스틱 점수와 달리 "실측" 신호를 제공합니다. (분석 엔진은 이식되어 있으나, Vercel AI Gateway 결제 활성화가 필요해 현재는 비활성)
- **GEO 리더보드**: 검증 모드의 실측 인용률을 기반으로 도메인을 랭킹 (검증 모드에 의존).
